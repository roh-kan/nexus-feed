  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'youtube') {
        ...
        if (!channelId) {
          throw new Error('Invalid YouTube channel URL. Please use the full channel URL or @username.');
        }
        finalUrl = channelId;
      }
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Source Type</label>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setType('rss')}
                className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-all ${type === 'rss' ? 'bg-blue-600/10 border-blue-500 text-blue-400' : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'}`}
              >
                RSS Feed
              </button>
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-all ${type === 'youtube' ? 'bg-red-600/10 border-red-500 text-red-400' : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-700'}`}
              >
                YouTube
              </button>
            </div>
