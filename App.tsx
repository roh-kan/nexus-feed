              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xl text-slate-400 font-medium leading-relaxed">
                    The intelligence-first content aggregator for the modern web.
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-all hover:-translate-y-1">
                <div className="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 mb-4 shadow-inner">
                  <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 5c7.18 0 13 5.82 13 13M6 11a7 7 0 017 7m-6 0a1 1 0 11-2 0 1 1 0 012 0z" /></svg>
                </div>
                <h3 className="text-white font-bold mb-1">Unified Feeds</h3>
                <p className="text-sm text-slate-400">Combine RSS and YouTube into a clean, minimalist timeline.</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-all hover:-translate-y-1">
                ...
                <p className="text-sm text-slate-400">Instant 15-word AI summaries to help you beat information overload.</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-all hover:-translate-y-1">
                <div className="w-12 h-12 rounded-xl bg-green-500/10 flex items-center justify-center text-green-500 mb-4 shadow-inner">
                  <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7c-2 0-3 1-3 3zM9 12h6M9 16h6M9 8h6" /></svg>
                </div>
                <h3 className="text-white font-bold mb-1">Sheet Sync</h3>
                <p className="text-sm text-slate-400">Your data belongs to you. No separate database - just your Google Sheets.</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-slate-700 transition-all hover:-translate-y-1">
                ...
                <p className="text-sm text-slate-400">Serverless architecture. We never store or track your content.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <aside className="hidden lg:flex w-72 flex-col bg-slate-900 border-r border-slate-800 p-6">
        <div className="flex items-center gap-3 mb-10">
          <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 flex items-center justify-between px-8 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <div className="flex bg-slate-800 rounded-lg p-1">
               <button onClick={() => setState(prev => ({...prev, filterReadStatus: 'all'}))} className={`px-3 py-1 text-xs font-semibold rounded-md ${state.filterReadStatus === 'all' ? 'bg-slate-700 text-white' : 'text-slate-500'}`}>All</button>
               <button onClick={() => setState(prev => ({...prev, filterReadStatus: 'unread'}))} className={`px-3 py-1 text-xs font-semibold rounded-md ${state.filterReadStatus === 'unread' ? 'bg-slate-700 text-white' : 'text-slate-500'}`}>Unread</button>
            </div>
            {state.isCloudSyncing && <span className="text-[10px] text-blue-400 font-bold uppercase tracking-widest animate-pulse">Syncing...</span>}
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
          <div className="max-w-4xl mx-auto space-y-4">
            ...
            {filteredItems.length === 0 && <div className="text-center py-20 text-slate-600">No items found.</div>}
          </div>
        </div>
      </main>
